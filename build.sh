...
mkdir -p build/obj
# every BearSSL source incl. the brssl tool, compiled once (musl), mtime-cached
n=0
...
echo "build.sh: compiled $n objects (cached rest)"
# brssl (vendored tool, static musl) turns anchors/*.pem into a const
# trust-anchor table (build/ta.h) at BUILD time: the daemon parses no
# X.509, it only validates the peer chain (x509-minimal) against it.
...
echo "build.sh: ${#} trust anchors -> build/ta.h"
